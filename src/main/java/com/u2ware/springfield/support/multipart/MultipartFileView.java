...
public class MultipartFileView extends AbstractView{
	...
	private DownloadFile filterModel(Map<String, Object> model)throws Exception {
		try{
			return (DownloadFile)ViewResolverSupport.getResponseModel(model);
		}catch(Exception e){
	...
	protected void download(HttpServletRequest request, HttpServletResponse response, Map<String, Object> model) throws Exception{
		...
		DownloadFile bean = filterModel(model);

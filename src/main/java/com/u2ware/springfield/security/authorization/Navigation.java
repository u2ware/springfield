...
public class Navigation implements Serializable{
	...
	public void travel(NavigationAccessor accessor){
		travel(0, accessor, this);
	}

	private void travel(int depth, NavigationAccessor accessor, Navigation n) {
		accessor.visit(n);
		
		if(CollectionUtils.isEmpty(n.getChildren())) return;
		int newDepth = depth +1;
		for(Navigation c : n.getChildren()){
			travel(newDepth, accessor, c);
		}
	}
